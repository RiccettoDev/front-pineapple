...
import ModalExistingTeam from "./components/modalExistingTeam";
...
export default function Teams() {
  ...
  const [modalExistingTeam, setModalExistingTeam] = useState(false)  
  ...
  function openModalExisting() {
    setModalExistingTeam(true)
    setOverlay(true)
  }
  ...
  return (
    <>
      ...
      >
        <Flex w={'100%'} justifyContent={"space-between"} direction={{base:"column", md:"row"}}>
          ...
          {user.role === 9 || user.role === 'admin' && (
            <Flex gap={4} justifyContent={"space-between"}>
              <Button color={'#0c0c32'} fontWeight={"bold"} bg={'#b9fa3c'} onClick={() => openModalCreate()}>
                Criar time
              </Button>
              <Button color={'#0c0c32'} fontWeight={"bold"} bg={'#b9fa3c'} onClick={() => openModalExisting()}>
                Times existentes
              </Button>
            </Flex>
          )}
        ...
        {modalExistingTeam && (
          <MyModal 
            isOpen={modalExistingTeam} 
            onClose={() => {
              setModalExistingTeam(false);
              setOverlay(false);
            }} 
            title={"Times Existentes"} 
            justifyContent="right"
            size="md"
          >
            <ModalExistingTeam 
              setModalExistingTeam={setModalExistingTeam} 
              setOverlay={setOverlay} 
              selectedGroupId={selectedGroupId} 
            />
          </MyModal>
